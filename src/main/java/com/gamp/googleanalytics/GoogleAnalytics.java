...
import static com.gamp.transport.DefaultRequestParameterDiscoverer.isEmpty;
import static com.gamp.transport.DefaultRequestParameterDiscoverer.isNotEmpty;
...
import com.gamp.transport.GoogleAnalyticsResponse;

/**
 * This is the main class of this library that accepts the requests from clients and
 * sends the events to Google Analytics (GA).
 *
 * Clients needs to instantiate this object with {@link GWAnalyticsConfig} and {@link DefaultRequest}.
...
public class GoogleAnalytics {
    ...
    private static final Logger logger = Logger.getLogger(GoogleAnalytics.class);
    ...
    public GoogleAnalytics(GoogleAnalyticsConfig config, String trackingId, String clientId, boolean isMobileApp, String appName,
                           String appVersion) {
    ...
    public GoogleAnalyticsResponse post(GoogleAnalyticsRequest request) {
        ...
        try {
            ...
            logger.debug("Response of the request " + response.getStatusCode() + " and posted params " + response.getPostedParms());
     ...
     * @param postParms
     */
    private void processCustomDimensionParameters(@SuppressWarnings("rawtypes") GoogleAnalyticsRequest request,
                                                  List<NameValuePair> postParms) {
    ...
    public Thread newThread(Runnable r) {
        Thread thread =
            new Thread(Thread.currentThread().getThreadGroup(),
                       r,
                       MessageFormat.format(threadNameFormat, threadNumber.getAndIncrement()),
                       0);
